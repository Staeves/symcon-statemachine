...
class Statemachine extends IPSModule {
		...
		foreach (json_decode($this->ReadAttributeString("transitions"), true) as $transition) {
			...
			if (str_starts_with($start, "state-")) {
				...
				if (!array_key_exists($trigger, $data[$start]["fromState"])) {
					...
					$problemState = json_decode($this->ReadAttributeString("states"), true)[$start]["name"];
				...
				foreach ($stateList as $state) {
					...
					if (!array_key_exists($trigger, $data[$state]["fromGroup"])) {
						...
						$problemState = json_decode($this->ReadAttributeString("states"), true)[$state]["name"];
	...
	public function GetVisualizationTile() {
		$vals = "<body><script> let states = " . $this->ReadAttributeString("states") . ";
				...
				let triggers = " . $this->ReadAttributeString("triggers") . ";
			</script>";
		return $vals . file_get_contents(__DIR__ . '/module.html') . "</body>";
	}
